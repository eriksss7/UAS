...
import com.example.ramadhan.jadwalsolat.GetAddressIntentService
import com.example.ramadhan.jadwalsolat.MainAdapterSholat.onSelectData
import com.google.android.gms.location.*
import kotlinx.android.synthetic.main.layout_date_pray.*
import kotlinx.android.synthetic.main.layout_jadwal_imsak.*
...
class MainSholat : AppCompatActivity(), onSelectData {

    var modelMainList: MutableList<ModelMainSholat> = ArrayList()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.layout_jadwal_sholat)
    ...
    private val textJSON: Unit
        get() {
            val progressDialog = ProgressDialog(this@MainSholat)
                        ...
                        override fun onResponse(response: JSONObject) {
                            try {
                                ...
                                for (i in 0 until playerArray.length()) {
                                    ...
                                    val jsonObject4 = jsonObject3.getJSONObject("month")
                                    val strYearMonth = jsonObject4.getString("en")
                                    txtDateHijri!!.text = jsonObject3.getString("day") + " " + strYearMonth + " " + strYear + " H"
                                }

                            } catch (e: JSONException) {
                                e.printStackTrace()
                                Toast.makeText(this@MainSholat, "Gagal menampilkan data!", Toast.LENGTH_SHORT).show()
                            }
                        }

                        override fun onError(anError: ANError) {
                            progressDialog.dismiss()
                            Toast.makeText(this@MainSholat, "Tidak ada jaringan internet!", Toast.LENGTH_SHORT).show()
                        }
                    })
        }
    ...
    private val dataJSON: Unit
        get() {
            ...
            progressDialog.setMessage("Mohon tunggu...")
            progressDialog.show()
            AndroidNetworking.get("http://api.aladhan.com/v1/hijriCalendarByCity?city=Jakarta&country=Indonesia&method=5&month=11&year=1441")
                    .setPriority(Priority.HIGH)
                    .build()
                    .getAsJSONObject(object : JSONObjectRequestListener {
                        override fun onResponse(response: JSONObject) {
                            try {
                                ...
                                for (i in 0 until playerArray.length()) {
                                    ...
                                    val jsonObject6 = jsonObject3.getJSONObject("month")
                                    val strYearMonth = jsonObject6.getString("en")
                                    ...
                                    dataApi.txtYearMonth = strYearMonth
                                    dataApi.txtDay = "$strDay $strYearMonth $strYear H"
    ...
    private fun showNews() {
        val mainAdapter = MainAdapterSholat(this@MainSholat, modelMainList, this)
        rvList!!.adapter = mainAdapter
    }

    override fun onSelected(modelMain: ModelMainSholat) {
        val intent = Intent(this@MainSholat, DetailActivitySholat::class.java)
    ...
    private val address: Unit
        get() {
            if (!Geocoder.isPresent()) {
                Toast.makeText(this@MainSholat, "Can't find current address, ", Toast.LENGTH_SHORT).show()
                return
            }
    ...
    private inner class LocationAddressResultReceiver internal constructor(handler: Handler?) : ResultReceiver(handler) {
        override fun onReceiveResult(resultCode: Int, resultData: Bundle) {
            ...
            if (resultCode == 1) {
                Toast.makeText(this@MainSholat,
                        "Address not found, ",
